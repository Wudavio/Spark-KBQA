...
def get_sim_model():
    sim_path = '/KgSpark/output/data/'
    sim_processor = preProcessor()
...
def semantic_match(model, question, attribute_list, answer_list, max_seq_length):
    ...
    for attribute in attribute_list:
        ...
        features.append(
            SimInputFeatures(input_ids, masks, token_type_ids)
        )
    ...
    dataloader = DataLoader(dataset, sampler=sampler, batch_size=512, num_workers=4, pin_memory=True)
    ...
    prediction_val = torch.max(all_logits, dim = -1)[0]
    ...
    # print(f"prediction_val: {prediction_val.argmax()}")

    if prediction.sum() == 0:
        return torch.tensor(0)
    else:
        return prediction_val.argmax()
...
def calc_acc(real_label, pred_label):
    ...
    # assert real_label.shape[0] == real_label[:,0].sum() #20221111
...
def test():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    args_test = {"data_dir": '/home/s21639102/KgCLUE/preprocess_dataset/sim_data',
                 "load_path": '/home/s21639102/KgCLUE/output/data/',
...
if __name__ == '__main__':
    ...
    args_train = {"data_dir": '/home/s21639102/KgCLUE/preprocess_dataset/sim_data',
            "pre_train_model": 'bert-base-chinese',
            "output_dir": '/home/s21639102/KgCLUE/output/data/',
            "max_seq_length": 128,
            "batch_size": 16,
            "learning_rate": 6e-6,
            "epochs": 1,
